...
EXTRA_FIELDS = ['name', 'email', 'lang', 'action', 'product', 'phone', 'street',
...
def message_parse(self, cr, uid, message, save_original=False, context=None):
    ...
    msg = message_parse_orig(self, cr, uid, message, save_original=save_original, context=context)
    ...
    if email:
        ...
        author_ids = partner_obj.search(cr, uid, [('email', 'ilike', email)], limit=1, context=context)
        if author_ids:
...
def message_post(self, cr, uid, thread_id, context, **message_dict):
    ...
    return message_post_orig(self, cr, uid, thread_id, context=context, **message_dict)
